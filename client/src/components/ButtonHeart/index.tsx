...
import { COLORS, DETAIL } from "../../utils/constants";

export const ButtonHeart = () => {
  const { detailVariables } = useDetail();
  const { authVariables } = useAuth();

  const { favoritesVariables, favoritesActions } = useFavorites();
  ...
  const isInFavorites = (favoriteAnimals: Array<ANIMALS_INTERFACE>) => {
    ...
    return isTheAnimalInFavorites;
  };

  useEffect(() => {
    if (favoritesVariables.favoriteAnimals.length > 0) {
      if (isInFavorites(favoritesVariables.favoriteAnimals)) {
        setIsFavorite(true);
  ...
  return (
    <button
      ...
      onClick={() =>
        isFavorite
          ? favoritesActions.deleteFavoriteAnimals(
              detailVariables?.detailAnimal?.id
            )
          : favoritesActions.postFavoriteAnimals(detailVariables?.detailAnimal)
      }
    >
      {authVariables.isLoading ? (
        <div role="status " style={{ width: "25px" }}>
          <svg
            className="inline mr-2 w-6 h-6 text-gray-200 animate-spin dark:text-purple-700 fill-white"
            viewBox="0 0 100 101"
            fill="none"
        ...
        <SvgDetail type={DETAIL.HEART} fill={color} />
      )}
    </button>
  );
};
